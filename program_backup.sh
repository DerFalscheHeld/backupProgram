...
mainDataFile="$configDir/main.json"
...
case $1 in
      ...
      case $key in 
        ...
        backupPath)
          if [[ -d $value ]] ; then
            trueValue=1
            if [[ -d $value ]] ; then 
              valueLength=$(echo $value | wc -L)
              if [[ ${value:$(($valueLength-1)):$(($valueLength-1))} = "/" ]] ; then
                value=$(echo ${value:0:$(($y-2))})
              fi
            fi
          else
            trueValue=0
  ...
  --restoreConfig)
    ...
    if [[ "$3" = "--yes" ]] ; then
      ...
      output -n "${magenta}Q & A?: ${yellow}Do you really want to ${red}delete ${yellow}the config files and restore from the file? ${reset}[${green}y${reset}/${red}N${reset}] : " ;
      read option
      case $option in
        yes|y|Yes|Y)
          if test -f $2 ; then
            output "\n${cyan}MSG   : ${yellow}Deleting config file..."
            rm -rf $mainDataFile
            output "${cyan}MSG   : ${yellow}Config file deleted!\n"
            output "${cyan}MSG   : ${yellow}Restore data from file..."
            touchData
            cp $2 $mainDataFile
            output "${cyan}MSG   : ${yellow}Restored!"
          else
            error "${red}Error : ${yellow}Config can't be restored, $2 is not a file."
          fi
          ;;
        *)    
          output "\n${cyan}MSG   : ${green}Config file not deleted!\n"
          ;;
      esac
    else
      error "${red}Error : ${yellow}'$3' is not an argument for --restoreConfig."
    fi
    ;;

  --deleteConfigFiles)
    if [[ $# -ge 3 ]] ; then
      error "${red}Error : ${yellow}To many arguments for --deleteConfigFiles."
      exit_
    fi
    if [[ "$2" = "--yes" ]] ; then
      rm -rf $programmDir
    ...
    ouput -n "${magenta}Q & A?: ${yellow}Do you really want to ${red}delete ${yellow}the configfile? ${reset}[${green}y${reset}/${red}N${reset}] : " ;
    read option
    case $option in
      yes|y|Yes|Y)  output "\n${cyan}MSG   : ${yellow}Deleting config file..."
                    rm -rf $programmDir
